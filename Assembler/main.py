...
programs=fun.ins_return(f"Tech.txt")
for main_program in programs:
    exit = "__exit__"
    if(main_program==[]):
        __name__= exit
    else:
    ...
    # binary address of lable
    labels=fun.ad_dict(main_program,program_memory)
    ...
    for i in main_program:  
        ...
        if(i[0] in a):
                ...
                if  temp_num <0 or temp_num>127:
                    set_of_errors.append(f"Error in line {machine_code.index(i)+1}: Imm must be a whole number: range (0-127)")
                    net_str="FC"
                    set_of_mcode.append(net_str)
                    continue
            ...
            set_of_errors.append(f"Error at line {main_program.index(i)+1+len(variables)}: variables must be declared in the begining")
                ...
                continue
                #for now it is pass to avoid error in testing
                #please use this block to create error cases.
    flag=0
    ...
    if(flag==0):
            machine_code="\n".join(set_of_mcode)
            machine_code+="\n\n"
            fun.write_machine(f"output.txt",machine_code)  
    else:
            errors="\n".join(set_of_errors)
            errors+="\n\n"
            fun.write_machine(f"output.txt",errors)